...
class ExcelToPdfWorkflow(BaseWorkflow):
    ...
    def get_instructions(cls):
        ...
        """
        return ("Started Excel to PDF conversion.\n"
                "Send me Excel spreadsheets (.xls or .xlsx files) and I'll convert them to PDF.\n"
                "Type 'done' when you've sent all spreadsheets to convert.")
    ...
    def handle_command(self, message_text, quoted_stanza_id):
        ...
        if message_text.lower() == "done":
            ...
            return True, "Processing your spreadsheets..."
        
        # Check for status command
        if message_text.lower() == "status":
            return self._check_tasks_status()
        
        return False, "Send me Excel spreadsheets (.xls or .xlsx files) to convert to PDF, or type 'done' to finish."
    ...
    def handle_file_save(self, message_id, saved_filename):
        ...
        try:
            ...
            # Generate output PDF filename using the original filename if available
            ...
            if filename_ext.lower() not in ['.xls', '.xlsx', '.xlsm']:
                logger.warning(f"Not an Excel spreadsheet: {saved_filename}")
                return saved_filename, f"File {saved_filename} is not an Excel spreadsheet. Please send a .xls, .xlsx, or .xlsm file."
            
            logger.info(f"Converting Excel spreadsheet to PDF: {file_path}")
            
            # Convert Excel spreadsheet to PDF asynchronously using Celery task
            task_result = execute_task(convert_document_task, file_path, self.task_dir, doc_type='excel')
            ...
            if hasattr(task_result, 'id'):  # It's an async task
                ...
                # Store temporary information in spreadsheet_versions
                if 'spreadsheet_versions' not in self.state:
                    self.state['spreadsheet_versions'] = {}
                
                self.state['spreadsheet_versions'][message_id] = {
                ...
                return saved_filename, (
                    f"Excel spreadsheet received. Converting to PDF in the background.\n"
    ...
    def finalize(self):
        ...
        try:
            ...
            if 'spreadsheet_versions' not in self.state or not self.state['spreadsheet_versions']:
                logger.warning("No spreadsheets received for conversion.")
                return []
            
            # Collect all PDF files
            for message_id, versions in self.state['spreadsheet_versions'].items():
                if 'pdf' in versions:
            ...
            if len(output_files) > 1:
                ...
                merged_pdf_path = os.path.join(self.task_dir, "Merged_Spreadsheets.pdf")
                writer = PdfWriter()
    ...
    def _check_tasks_status(self):
        ...
        completed_count = 0
        failed_count = 0
        status_message = "Excel to PDF conversion status:\n"
        ...
        for message_id, task_id in self.state["task_ids"].items():
            try:
                ...
                original_filename = self.get_original_filename(message_id, f"spreadsheet_{message_id}")
                ...
                if status in ['PENDING', 'STARTED', 'RETRY']:
                    ...
                    if message_id in self.state.get("spreadsheet_versions", {}) and \
                       self.state["spreadsheet_versions"][message_id].get("conversion_status") == "pending":
                        try:
                            # Get task result
                            ...
                            if output_pdf_path and os.path.exists(output_pdf_path):
                                ...
                                expected_pdf = self.state["spreadsheet_versions"][message_id].get("expected_pdf")
                                ...
                                self.state["spreadsheet_versions"][message_id].update({
                                    'pdf': os.path.basename(output_pdf_path),
                                    'conversion_status': 'completed'
                                })
                                ...
                                self.state["spreadsheet_versions"][message_id]['conversion_status'] = 'failed'
                    ...
                    # Update spreadsheet_versions if needed
                    if message_id in self.state.get("spreadsheet_versions", {}):
                        self.state["spreadsheet_versions"][message_id]['conversion_status'] = 'failed'
                    
            except Exception as e:
                logger.error(f"Error checking task status for {message_id}: {str(e)}")
                status_message += f"- Task for spreadsheet {message_id}: ERROR checking status\n"
    ...
    def _wait_for_pending_tasks(self):
        ...
        for message_id, task_id in self.state["task_ids"].items():
            ...
            if message_id in self.state.get("spreadsheet_versions", {}) and \
               self.state["spreadsheet_versions"][message_id].get("conversion_status") == "completed":
                continue
