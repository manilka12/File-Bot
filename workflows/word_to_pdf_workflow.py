...
class WordToPdfWorkflow(BaseWorkflow):
    ...
    def get_instructions(cls):
        ...
        """
        return ("Started Word to PDF conversion.\n"
                "Send me Word documents (.doc or .docx files) and I'll convert them to PDF.\n"
                "Type 'done' when you've sent all documents to convert.")
    ...
    def handle_command(self, message_text, quoted_stanza_id):
        ...
        if message_text.lower() == "done":
            ...
            return True, "Processing your documents..."
        
        # Check for status command
        if message_text.lower() == "status":
            return self._check_tasks_status()
        
        return False, "Send me Word documents (.doc or .docx files) to convert to PDF, or type 'done' to finish."
    ...
    def finalize(self):
        ...
        try:
            ...
            if 'document_versions' not in self.state or not self.state['document_versions']:
                logger.warning("No documents received for conversion.")
                return []
            
            # Collect all PDF files
            for message_id, versions in self.state['document_versions'].items():
                if 'pdf' in versions:
            ...
            if len(output_files) > 1:
                ...
                merged_pdf_path = os.path.join(self.task_dir, "Merged_Documents.pdf")
                writer = PdfWriter()
    ...
    def handle_file_save(self, message_id, saved_filename):
        ...
        return self._handle_document_save(message_id, saved_filename)
    
    def _handle_document_save(self, message_id, saved_filename):
        """
        ...
        try:
            ...
            # Convert Word document to PDF asynchronously using Celery task
            task_result = execute_task(convert_document_task, file_path, self.task_dir, doc_type='word')
            ...
            if hasattr(task_result, 'id'):  # It's an async task
                ...
                # Store temporary information in document_versions
                if 'document_versions' not in self.state:
                    self.state['document_versions'] = {}
                
                self.state['document_versions'][message_id] = {
                ...
                return saved_filename, (
                    f"Word document received. Converting to PDF in the background.\n"
    ...
    def _check_tasks_status(self):
        ...
        completed_count = 0
        failed_count = 0
        status_message = "Word to PDF conversion status:\n"
        ...
        for message_id, task_id in self.state["task_ids"].items():
            try:
                ...
                original_filename = self.get_original_filename(message_id, f"document_{message_id}")
                ...
                if status in ['PENDING', 'STARTED', 'RETRY']:
                    ...
                    if message_id in self.state.get("document_versions", {}) and \
                       self.state["document_versions"][message_id].get("conversion_status") == "pending":
                        try:
                            # Get task result
                            ...
                            if output_pdf_path and os.path.exists(output_pdf_path):
                                ...
                                expected_pdf = self.state["document_versions"][message_id].get("expected_pdf")
                                ...
                                self.state["document_versions"][message_id].update({
                                    'pdf': os.path.basename(output_pdf_path),
                                    'conversion_status': 'completed'
                                })
                                ...
                                self.state["document_versions"][message_id]['conversion_status'] = 'failed'
                    ...
                    # Update document_versions if needed
                    if message_id in self.state.get("document_versions", {}):
                        self.state["document_versions"][message_id]['conversion_status'] = 'failed'
                    
            except Exception as e:
                logger.error(f"Error checking task status for {message_id}: {str(e)}")
                status_message += f"- Task for document {message_id}: ERROR checking status\n"
    ...
    def _wait_for_pending_tasks(self):
        ...
        for message_id, task_id in self.state["task_ids"].items():
            ...
            if message_id in self.state.get("document_versions", {}) and \
               self.state["document_versions"][message_id].get("conversion_status") == "completed":
                continue
